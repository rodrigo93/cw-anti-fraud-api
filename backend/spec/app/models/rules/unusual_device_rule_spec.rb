# spec/models/rules/unusual_device_rule_spec.rb
...
RSpec.describe Rules::UnusualDeviceRule, type: :model do
  describe '#apply' do
    ...
    context 'when transaction device is not present' do
      let(:transaction) { create :transaction, user_id: user_id, device_id: nil }
    ...
    context 'when user has other transactions with different devices' do
      before { create :transaction, user_id: user_id, device_id: 'other_device' }
    ...
    context 'when user has other transactions with the same device' do
      before { create :transaction, user_id: user_id, device_id: transaction.device_id }

      it { is_expected.to be_truthy }
    end

    context 'when user has other transactions with the same device but different transaction_id' do
      before { create :transaction, user_id: user_id, device_id: 'device1' }

      let(:transaction) { create :transaction, user_id: user_id, device_id: 'device1' }
